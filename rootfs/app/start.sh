...
if [ -f /app/config/logback.xml ] && [ -f /app/config/server.log ]; then
  $JAVA -Xmx256m -cp ctbrec-server-5.0.2-final.jar -Dfile.encoding=utf-8 -Dctbrec.config.dir=/app/config -Dctbrec.config=server.json ctbrec.recorder.server.HttpServer >> /app/config/server.log
else
  $JAVA -Xmx256m -cp ctbrec-server-5.0.2-final.jar -Dfile.encoding=utf-8 -Dctbrec.config.dir=/app/config -Dctbrec.config=server.json ctbrec.recorder.server.HttpServer
fi
